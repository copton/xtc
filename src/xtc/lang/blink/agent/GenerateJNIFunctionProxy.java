...
public final class GenerateJNIFunctionProxy implements JNIConstants {
  ...
  private void genHeaders() throws IOException{
    w.printf(
  "/* This source file is generated by " +
  GenerateJNIFunctionProxy.class.getSimpleName() + ".java."+ 
  ...
  "  assert(err == JVMTI_ERROR_NONE);\n");
      for(final ProxyGenerator p : pgs) {
        String jname = p.getJNIFuncName();
        String pname = p.getProxyFuncName();
...
"}\n");
  ...
  static String getFieldDescFromType(String s) {
   ...
   static final HashMap<String,String> type2jvalueFieldName = new HashMap<String,String>();
    ...
    final void genStatReport(PrintWriter w) {
      w.printf("  printf(\"%%7d %%7d %%s\\n\", %s, %s, \"%s\");", 
               getStatCountName(), getStatCountUserName(), getJNIFuncName());
      w.printf("  sum += %s;\n", getStatCountName());
    ...
    void genCallCounts(PrintWriter w) {
    ...
    void genJNICheckBefore(PrintWriter w) {
      ...
      boolean exceptionCheck = !JNIFunction.isExceptionHazardFree(wrapperJNIFunction);
      Matcher m;
    
      // check the validity of incoming arguments.
      w.printf("  /* Check the JNI Function call. */\n");
      w.printf("  if (bda_options.jnicheck && (s != NULL) && (s->mode != JVM) && !bda_is_in_jdwp_region(ret_addr)) {\n");
      ...
      if (wrapperJNIFunction.hasReferenceReturn()) {
        w.printf("    && bda_check_capacity_c2j_call(s, \"%s\")\n", jniFuncName);
      }
      ...
      if (exceptionCheck) {
        for(int i = 1; i < fargs.length;i++) {
          ...
          if (t.jniType instanceof ReferenceType){
            w.printf("    && bda_check_dref(s, %s, %d, \"%s\")\n", f.varName, i, jniFuncName);
      ...
      if (jniFuncName.equals("DefineClass")) {
        w.printf("    && bda_check_subclass_jobject_jclass(s, %s, bda_clazz_classloader, %d, \"%s\")\n", 
        ...
        w.printf("    && bda_check_subclass_jobject_jclass(s, %s, bda_clazz_nio_buffer, %d, \"%s\")\n",
                 fargs[1].varName, 1, jniFuncName);
      } else if (jniFuncName.equals("GetDirectBufferCapacity")) {
        w.printf("    && bda_check_subclass_jobject_jclass(s, %s, bda_clazz_nio_buffer, %d, \"%s\")\n",
                 fargs[1].varName, 1, jniFuncName);
      } else if (jniFuncName.equals("ThrowNew")) {
        w.printf("    && bda_check_subclass_jclass_jclass(s, %s, bda_clazz_throwable, %d, \"%s\")\n", 
                 fargs[1].varName, 1, jniFuncName);
      } else if (jniFuncName.equals("AllocObject")) {
        w.printf("    && bda_check_jclass_scalar_allocatable(s, %s, %d, \"%s\")\n", 
                  fargs[1].varName, 1, jniFuncName);
      }
//       } else if (jniFuncName.equals("GetPrimitiveArrayCritical")) {
//         w.printf("    && bda_check_jarray_primitive(s, %s, %d, \"%s\")\n", 
//                  fargs[1].varName, 1, jniFuncName);
//       } else if (jniFuncName.equals("ReleasePrimitiveArrayCritical")) {
//         w.printf("    && bda_check_jarray_primitive(s, %s, %d, \"%s\")\n", 
//                  fargs[1].varName, 1, jniFuncName);
//       }
      for(int i = 1; i < fargs.length;i++) {
    ...
    void genTracerPrologue(PrintWriter w) {
      PrintfFormatAndArgument f = getFixedArgumentPrintFormatAndArgumentExpression();
      w.printf("  /* Trace Prologue. */\n");
      w.printf("  if (bda_options.mname[0] && bda_trace_match(\"%s\", s->name)) {\n", getJNIFuncName());      
      w.printf("    printf(\"=> %%-50s %s\\n\", \"%s\" %s);\n", f.fmt, getJNIFuncName(), f.list);
      switch(wrapperJNIFunction.extraArgType) {
      case VA_LIST:{
        FormalArgument vaListArg = fargs[fargs.length-1]; 
        assert vaListArg.type.jniType == JNIConstants.VA_LIST;
        String methodidVariableName = findMethodIDVariable().varName;
        String argName = vaListArg.varName;
        w.printf("    bda_jmethod_print_argument_from_va_list(%s, %s, %s);\n", 
            fargs[0].varName, methodidVariableName, argName);
        break;
      }
      case JVALUE_ARRAY: { 
        FormalArgument jvaluePointerArg = fargs[fargs.length -1 ];
        assert jvaluePointerArg.type.jniType == JNIConstants.JVALUE_POINTER;
        String methodidVariableName = findMethodIDVariable().varName;
        String argName = jvaluePointerArg.varName;
        w.printf("    bda_jmethod_print_argument_from_array(%s, %s, %s);\n",
            fargs[0].varName, 
            methodidVariableName, argName);
        break;
      }
        case DOT_DOT_DOT: {
        String methodidVariableName = findMethodIDVariable().varName;
        String lastArgumentName = fargs[fargs.length-1].varName;
        w.printf("    va_start(args,%s);\n", lastArgumentName);
        w.printf("    bda_jmethod_print_argument_from_va_list(%s, %s, args);\n",
            fargs[0].varName, methodidVariableName);
        break;
        }
      }
      w.printf("  }\n");
    ...
    void genTracerEpilogue(PrintWriter w) {
      w.printf("  /* Trace Epilogue. */\n");
      JNIType returnType = targetJNIFunction.returnType.jniType;
      if ( returnType == JNIConstants.VOID) {
        w.printf("  if (bda_options.mname[0] && bda_trace_match(\"%s\", s->name)) {\n", getJNIFuncName());
        w.printf("    printf(\"<= %%-50s\\n\", \"%s\");\n", getJNIFuncName());
        w.printf("  }\n");
      } else {
        String fmtReturn = getCformatStringFor(returnType);
        w.printf("  if (bda_options.mname[0] && bda_trace_match(\"%s\", s->name)) {\n", getJNIFuncName());
        w.printf("    printf(\"<= %%-50s %s\\n\", \"%s\", result);\n", fmtReturn,  getJNIFuncName());
        w.printf("  }\n"); 
      }
      w.printf("\n");
    }
    ...
    void genJNICheckAfter(PrintWriter w) {
      ...
      w.printf("  if ((s != NULL) && bda_options.jnicheck && (s->mode != JVM)) {\n");  
      if (wrapperJNIFunction.hasReferenceReturn()) {
        ...
        } else if (wrapperJNIFunction == JNIFunction.NewLocalRef) {
          w.printf("      if (result != NULL) {bda_local_ref_add(s, result);}\n");
        } else {
          w.printf("      if (result != NULL) {bda_local_ref_add(s, result);}\n");
        }
        w.printf("    }\n");
        ...
        w.printf("   if (%s != NULL) {bda_local_ref_delete(s, %s);}\n", fargs[1].varName,fargs[1].varName);
      ...
      } else if (wrapperJNIFunction == JNIFunction.PushLocalFrame) {
        w.printf("   if (result == 0) {\n");
        w.printf("     bda_local_ref_enter(s, %s, 0);\n", fargs[1].varName);
        w.printf("   }\n");
      } else if (wrapperJNIFunction == JNIFunction.PopLocalFrame) {
        w.printf("   bda_local_ref_leave(s);\n");
        w.printf("   if (result != NULL) {\n");
        w.printf("     bda_local_ref_add(s, result);\n");
        w.printf("   }\n");
      } else if (wrapperJNIFunction == JNIFunction.GetMethodID) {
        w.printf("   if (result != NULL) {\n");
      ...
      // epilogue action
      if (jniFuncName.matches("Release(String|PrimitiveArray)Critical")) {
        w.printf("    bda_leave_critical(s);\n");
      }
