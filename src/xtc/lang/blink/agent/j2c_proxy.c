...
static void bda_j2c_trace_before(JNIEnv * env, jobject classOrObject, va_list argp, const struct bda_java_method_info *  minfo)
{
    int i;
    printf("=> %-50s %p(JNIEnv*) %p(%s)", 
           minfo->fullname, env, 
           classOrObject, minfo->is_static? "jclass":"jobject");
    for (i=0;i< minfo->num_arguments;i++) {
      const struct bda_java_argument_info * a = &(minfo->arguments[i]);
      switch(a->type) {
      case 'Z': case 'B': case 'C': case 'I': case 'S': {
        jint v = va_arg(argp, jint);
        printf(" %d(%s)", v, bda_jmethod_primitive_name(a->type));
        break;
      }
      case 'J': {
        jint v1 = va_arg(argp, jint);
        jint v2 = va_arg(argp, jint);
        printf(" %x.%x(%s)", v1, v2, bda_jmethod_primitive_name(a->type));
        break;
      }
      case 'F': {
        jint v = va_arg(argp, jint);
        printf(" %x(%s)", v, bda_jmethod_primitive_name(a->type));
        break;
      }
      case 'D': {
        jint v1 = va_arg(argp, jint);
...
static void bda_j2c_trace_after(JNIEnv * env, const struct bda_java_method_info *  minfo, 
                                int return_lsb, int return_msb)
{
      char type = minfo->returnInfo.type;
      printf("<= %-50s", minfo->fullname);

      switch (type) {
      case 'Z': case 'B': case 'C': case 'I': case 'S': case 'F': {
          printf(" %d(%s)",  return_lsb, bda_jmethod_primitive_name(type));
          break;
      }
      case 'J': case 'D': {
          printf("  %x.%x(%s)", return_lsb, return_msb, bda_jmethod_primitive_name(type));
          break;
      }
      case 'L': case '[': {
          printf(" %p(%s)", (jobject)return_lsb, "jobject");
          break;
      }
      case 'V': {break;}
...
#error "Unsupported"
  ...
  /* trace. */
  if ((s != NULL) && bda_options.mname[0] && bda_trace_match(minfo->fullname, s->name)) {
    va_list argp;
    va_start(argp, classOrObject);
    bda_j2c_trace_before(env, classOrObject, argp, minfo);
    va_end(argp);
  }
...
#error "Unsupported"
  ...
  if (s != NULL) {
    ...
    /* trace */
    if (bda_options.mname[0] && bda_trace_match(minfo->fullname, s->name)) {
        bda_j2c_trace_after(env, minfo, return_eax, return_edx);
    }

    if (bda_options.jnicheck) {
        int success = 1
            && bda_check_capacity_j2c_return(s);
...
void bda_j2c_proxy_dump_stat()
{
    ...
    for(i=0;i<num_bindings;i++){
        ...
        printf("%10d %10d %s%s\n", bind->j2c_count, bind->j2c_count_user, minfo->fullname, minfo->mdesc);
        sum += bind->j2c_count;
...
static void bda_analyze_native_method(jvmtiEnv *jvmti, struct native_method_bind * bind)
{
  ...
  if (bda_options.verbose >= 3) {
    jvmtiPhase phase;
    err = (*jvmti)->GetPhase(jvmti, &phase);
    assert(err == JVMTI_ERROR_NONE);
    if (phase == JVMTI_PHASE_LIVE) {
      printf("bda_j2c_proxy_add: id=%d methodid = %p address = %p\n",
             bind->proxy_id, method, bind->original_native_method_address);
    } else {
      printf("bda_j2c_proxy_add: %s %s id=%d methodid = %p address = %p\n",
             minfo->fullname, minfo->mdesc, bind->proxy_id, minfo->mid,
             bind->original_native_method_address);
    }
  }
}
