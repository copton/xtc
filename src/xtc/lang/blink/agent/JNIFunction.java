...
final class JNIFunction implements JNIConstants {
   ...
   */
  public static boolean isExceptionHazardFree(JNIFunction f) {
    
    if ((f == ExceptionOccurred) || (f == ExceptionDescribe)
        || (f == ExceptionClear) || (f == ExceptionCheck)
        || (f == ReleaseStringChars) || (f == ReleaseStringUTFChars)
        || (f == ReleaseStringCritical) || (f == ReleaseBooleanArrayElements)
        || (f == ReleaseByteArrayElements) || (f == ReleaseCharArrayElements)
        || (f == ReleaseShortArrayElements) || (f == ReleaseIntArrayElements)
        || (f == ReleaseLongArrayElements) || (f == ReleaseFloatArrayElements)
        || (f == ReleaseDoubleArrayElements)
        || (f == ReleasePrimitiveArrayCritical) || (f == DeleteLocalRef)
        || (f == DeleteGlobalRef) || (f == DeleteWeakGlobalRef)
        || (f == MonitorExit)) {
      return true;
    } else {
      return false;
  ...
  public static final JNIFunction DefineClass = JNIFunction.create(5, false, "DefineClass",JNIFunction.t(JCLASS), JNIFunction.nt(UTF8_CLASS_DESC), JNIFunction.nt(JOBJECT), JNIFunction.nt(JBYTE_CONST_POINTER),JNIFunction.t(JSIZE));
