...
{
    jint jni_result;
    jclass agent_localref, agent_variable_localref;

    /* obtain a agent class reference. */
    agent_localref = bda_orig_jni_funcs->DefineClass(env, AGENT_CLASS_NAME, NULL,
                                         class_agent, sizeof(class_agent));
    if (agent_localref == NULL) {
        bda_orig_jni_funcs->ExceptionDescribe(env);
    }
    assert(agent_localref != NULL);
    agent_class = bda_orig_jni_funcs->NewGlobalRef(env, agent_localref);
    assert(agent_class != NULL);

    /* obtain a agent variable class reference. */
    agent_variable_localref = bda_orig_jni_funcs->DefineClass(env, AGENT_VARIABLE_CLASS_NAME,
                                                  NULL, class_agent_variable,
                                                  sizeof(class_agent_variable));
    assert(agent_variable_localref != NULL);
    agent_variable_class = bda_orig_jni_funcs->NewGlobalRef(env, agent_variable_localref);
    assert(agent_variable_class != NULL);

    /* obtain agent Java method identifiers. */
    bda_cache_static_java_method_id(env, agent_class, &agent_java_method_init);
    bda_cache_static_java_method_id(env, agent_class, &agent_java_method_jbp);
    bda_cache_static_java_method_id(env, agent_class, &agent_java_method_dummy_java);
    bda_cache_static_java_method_id(env, agent_variable_class, &agent_var_set_vj_boolean);
    bda_cache_static_java_method_id(env, agent_variable_class, &agent_var_set_vj_int);
    bda_cache_static_java_method_id(env, agent_variable_class, &agent_var_set_vj_double);
    bda_cache_static_java_method_id(env, agent_variable_class, &agent_var_set_vj_object);
    bda_cache_static_java_method_id(env, agent_variable_class, &agent_var_get_vj_boolean);
    bda_cache_static_java_method_id(env, agent_variable_class, &agent_var_get_vj_int);
    bda_cache_static_java_method_id(env, agent_variable_class, &agent_var_get_vj_double);
    bda_cache_static_java_method_id(env, agent_variable_class, &agent_var_get_vj_object);
...
JNIEXPORT void JNICALL bda_j2c(JNIEnv *env, jclass clz)
{
    ...
    /* trigger break point. */
    bda_cbp(J2C_DEBUGGER, env, NULL, NULL);
...
void bda_cbp(breakpoint_type bptype,
             JNIEnv *env,
...
void bda_get_c2j_target(struct bda_c2j_info * c2j_info, struct java_source_location *jloc)
{
    enum bda_c2j_call_type c2j_type = c2j_info->call_type;
    JNIEnv *env = c2j_info->env;
